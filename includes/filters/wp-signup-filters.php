...
/**
 * Add username-specific validation errors for generated signup usernames.
 */
function loopis_theme_hq_add_signup_username_validation_errors($errors, $generated_username) {
    // Allow lowercase letters and numbers separated by single hyphens.
    if (!preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $generated_username)) {
        $errors->add('user_name', __('This username is invalid. Please use only lowercase letters, numbers, and hyphens.', 'loopis-theme-hq'));
    }

    if (strlen($generated_username) < 4) {
        $errors->add('user_name', __('Username must be at least 4 characters long after generation.', 'loopis-theme-hq'));
    }

    $illegal_logins = (array) apply_filters('illegal_user_logins', array());
    if (in_array($generated_username, $illegal_logins, true)) {
        $errors->add('user_name', __('Sorry, that username is not allowed.', 'loopis-theme-hq'));
    }
}
...
function loopis_theme_hq_validate_signup_name_fields($result) {
    ...
    loopis_theme_hq_add_signup_username_validation_errors($result['errors'], $generated_username);
...
/**
 * Disable default multisite verification email
 */
add_filter('wpmu_signup_user_notification', '__return_false');

/**
 * Send customized multisite verification email
 */
function loopis_theme_hq_send_custom_signup_verification_email($user_login, $user_email, $key, $meta) {
    ...
    include_once LOOPIS_THEME_HQ_DIR . '/includes/functions/mail/loopis-signup-activation-mail.php';
    ...
    // Build mail
    $message = loopis_signup_activation_mail($first_name, $activation_url);
    $message .= loopis_mail_footer();
    $subject = '🔔 Verifiera ditt konto på LOOPIS';
...
function loopis_theme_hq_send_custom_welcome_user_notification($user_id, $password, $meta) {
    ...
    include_once LOOPIS_THEME_HQ_DIR . '/includes/functions/mail/loopis-mail-headers.php';
    include_once LOOPIS_THEME_HQ_DIR . '/includes/functions/mail/loopis-signup-welcome-mail.php';
    include_once LOOPIS_THEME_HQ_DIR . '/includes/functions/mail/loopis-mail-footer.php';

    $first_name = !empty($meta['first_name']) ? sanitize_text_field((string) $meta['first_name']) : '';
    $username = isset($user->user_login) ? (string) $user->user_login : '';

    $message = loopis_signup_welcome_mail($first_name, $username, $decrypted_password);
    $message .= loopis_mail_footer();

    $subject = '💚 Välkommen till LOOPIS!';
    wp_mail((string) $user->user_email, wp_specialchars_decode($subject), $message, loopis_mail_headers());
}
